...
def ga_dynamic_single(avg_fit, best_fit, global_fitness, max_frame=100):
    ...
    def animate(i):
        ...
        y1.append((l1[i]))
        y2.append((l2[i]))
        y3.append([global_fitness for x in avg_fit])

        axes.plot(x1, y1, "*", color="red")
        axes.plot(x1, y2, ".", color="blue")
        axes.plot(x1, y3, ".", color="green")
        axes.legend(["Average Fitness", "Best Fitness", "Global Fitness(Exact Optimization)"])
        axes.title.set_text(f"Best: {np.round(l2[i], 3)}, Average: {np.round(l1[i], 3)}, Global:{global_fitness}")
